...
function ServicesDropdown() {
  ...
  return (
    <div className="relative group">
      <a href="#services" className="hover:text-white inline-flex items-center gap-1">
        Services <ChevronDown className="h-4 w-4" />
      </a>
      <div className="absolute left-0 top-full mt-2 hidden group-hover:block">
        <div className="min-w-[240px] rounded-xl border border-zinc-800 bg-zinc-950/95 backdrop-blur shadow-xl p-2">
          {items.map((i) => (
            <a key={i.href} href={i.href} className="block px-3 py-2 rounded-md text-zinc-200 hover:bg-zinc-800/60">
              {i.label}
            </a>
          ))}
        </div>
